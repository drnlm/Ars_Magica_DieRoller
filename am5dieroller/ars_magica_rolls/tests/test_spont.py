...
def test_spont_success():
    ...
    # Should get 30 / 5 >= 5
    _roll, total, result, outcome = spont_non_roll(30, 5)
    assert total == 30
    assert result == 6
...
def test_spont_exact_success():
    ...
    # Should get 25 / 5 == 5
    _roll, total, result, outcome = spont_non_roll(25, 5)
    assert total == 25
    assert result == 5
    assert outcome == "success"
...
def test_spont_fail():
    ...
    # Should get 30 / 5 < 10
    _roll, total, result, outcome = spont_non_roll(30, 10)
    assert total == 30
    assert result == 6
...
def test_spont_neg_1_failure():
    ...
    # Should get 24 / 5 < 5
    _roll, total, result, outcome = spont_non_roll(24, 5)
    assert total == 24
    assert result == 4
    assert outcome == "failure"
...
def test_fatiguing_spont_success():
    ...
    with patch('random.randint', new_callable=Mock, side_effect=[6]):
        # should get 31 / 2 > 5
        _roll, total, result, outcome = fatiguing_spont_roll(25, 5)
        assert total == 31
        assert result == 15
        assert outcome == 'success'
...
def test_fatiguing_spont_fail():
    ...
    with patch('random.randint', new_callable=Mock, side_effect=[2]):
        # should get 27 / 2 < 15
        _roll, total, result, outcome = fatiguing_spont_roll(25, 15)
        assert total == 27
        assert result == 13
        assert outcome == 'failure'
...
def test_fatiguing_spont_fail_neg_1():
    ...
    with patch('random.randint', new_callable=Mock, side_effect=[2]):
        # should get 29/2 < 15
        _roll, total, result, outcome = fatiguing_spont_roll(27, 15)
        assert total == 29
        assert result == 14
        assert outcome == 'failure'
